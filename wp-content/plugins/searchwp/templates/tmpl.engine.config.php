...
function searchwpEngineSettingsTemplate( $engine = 'default' ) {
	...
	$indexed_post_types = apply_filters( 'searchwp_indexed_post_types', 'any' );
			...
			<?php if( $post_type->name != 'attachment' ) : ?>
				...
				<li data-swp-engine="swp-engine-<?php echo $engine; ?>-<?php echo $post_type->name; ?>" class="">
					<span>
						...
						<label for="swp_engine_<?php echo $engine; ?>_<?php echo $post_type->name; ?>" title="<?php echo esc_attr( $post_type->labels->name ); ?>"><?php echo esc_html( $post_type->labels->name ); ?></label>
					</span>
				</li>
			<?php endif; ?>
		<?php endforeach; ?>
					...
					<table>
						...
						<tbody>
							...
							<?php if( 'attachment' == $post_type->name ) : ?>
									...
									if( isset( $engineSettings[$post_type->name]['weights']['cf'] ) && is_array( $engineSettings[$post_type->name]['weights']['cf'] ) && !empty( $engineSettings[$post_type->name]['weights']['cf'] ) ) {
										...
										foreach( $cfWeights as $cfFlag => $cfWeight ) {
											if( $cfWeight['metakey'] == SEARCHWP_PREFIX . 'pdf_metadata' ) {
												$pdf_meta_weight = $cfWeight['weight'];
												break;
											}
										}
									}
								...
								<tr class="swp-custom-field">
									<td class="swp-custom-field-select">
										<label for="swp_engine_<?php echo $engine; ?>_<?php echo $post_type->name; ?>_<?php echo $arrayFlag; ?>_weight"><?php _e( 'PDF metadata (when applicable)', 'searchwp' ); ?></label>
									</td>
									<td>
										<input type="hidden" style="display:none;" name="<?php echo SEARCHWP_PREFIX; ?>settings[engines][<?php echo $engine; ?>][<?php echo $post_type->name; ?>][weights][cf][<?php echo $arrayFlag; ?>][metakey]" value="<?php echo SEARCHWP_PREFIX; ?>pdf_metadata" />
										<input type="number" min="-1" step="0.1" class="small-text" name="<?php echo SEARCHWP_PREFIX; ?>settings[engines][<?php echo $engine; ?>][<?php echo $post_type->name; ?>][weights][cf][<?php echo $arrayFlag; ?>][weight]" id="swp_engine_<?php echo $engine; ?>_<?php echo $post_type->name; ?>_<?php echo $arrayFlag; ?>_weight" value="<?php echo $pdf_meta_weight; ?>" />
									</td>
								</tr>
							<?php endif; ?>
