		...
		if ( ! empty( $this->purgeQueue ) ) {
			...
			if ( $this->alternate_indexer ) {
				...
				foreach ( $this->purgeQueue as $post_to_purge ) {
					$this->purgePost( intval( $post_to_purge ) );
				}
	...
	function advancedSettings() {
		...
		<div class="wrap">
			...
			<?php if ( $database_tables_recreated ) : ?>
				<div id="setting-error-settings_updated" class="updated">
					<p><?php _e( 'Database tables created!', 'searchwp' ); ?> <a href="options-general.php?page=searchwp"><?php _e( 'Initiate reindex', 'searchwp' ); ?></a></p>
				</div>
			<?php endif; ?>
			...
			<?php if ( ! $valid_database_environment ) : ?>
				...
				<p style="padding-bottom:23px;">
					...
					<a style="margin-left:13px;" class="button" id="swp-recreate-database-tables" href="options-general.php?page=searchwp&amp;nonce=<?php echo urlencode( $nonce ); ?>&amp;action=<?php echo urlencode( wp_create_nonce( 'swprecreatetables' ) ); ?>"><?php _e( 'Recreate Tables', 'searchwp' ); ?></a>
				</p>
			<?php endif; ?>
					...
					<script type="text/javascript">
						jQuery(document).ready(function($){
							$.post( ajaxurl, {
								...
								swpnonce: '<?php echo esc_attr( $_REQUEST['swpnonce'] ); ?>' },
								function(res){
									if ( res.hash ) {
