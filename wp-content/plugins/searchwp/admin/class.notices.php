		...
		if ( ! empty( $missing_integrations ) && apply_filters( 'searchwp_missing_integration_notices', true ) ) { ?>
			<?php foreach( $missing_integrations as $missing_integration ) : ?>
				...
				<div class="error" id="searchwp-missing-integrations-notice">
					<p><strong><?php _e( 'Missing SearchWP integration', 'searchwp' ); ?>:</strong> <?php echo sprintf( __( 'In order for SearchWP to work with %s you will need to install and activate the <a href="%s">%s</a> Extension.', 'searchwp' ), $plugin, $url, $integration ); ?> <?php echo sprintf( __( 'To dismiss this notice please see <a href="%s">these docs</a>.', 'searchwp' ), 'https://searchwp.com/?p=31517' ); ?></p>
				</div>
			<?php endforeach; ?>
		<?php }
	}
