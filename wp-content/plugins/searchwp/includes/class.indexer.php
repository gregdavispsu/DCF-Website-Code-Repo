...
class SearchWPIndexer {
		...
		if ( get_option( 'searchwp_transient' ) !== $hash ) {
			...
			if ( empty( $running ) ) {
				...
				if ( $this->findUnindexedPosts() !== false ) {
					...
					$destination = esc_url( $searchwp->endpoint ) . '?swpnonce=' . $this->hash;
						...
						if ( ! $skipDocProcessing && ! $omitDocProcessing ) {
							...
							if ( $this->post->post_mime_type == 'application/pdf' ) {
								...
								if ( file_exists( $filename ) && empty( $pdfContent ) ) {
									...
									if ( apply_filters( 'searchwp_index_pdf_metadata', true ) ) {
	...
	function preProcessTerms( $termsArray = array() ) {
		...
		if ( ( is_wp_error( $insert_result ) || false === $insert_result ) ) {
			...
			update_post_meta( $this->post->ID, '_' . SEARCHWP_PREFIX . 'attempts', $this->maxAttemptsToIndex + 1 );
			update_post_meta( $this->post->ID, '_' . SEARCHWP_PREFIX . 'skip', true );
