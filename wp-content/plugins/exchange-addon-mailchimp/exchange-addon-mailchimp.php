...
function it_exchange_register_mailchimp_addon() {
	...
	if ( true || version_compare( $current_version, '1.0.3', '>' ) ) {
		...
		$options = array(
			'name'              => __( 'MailChimp', 'it-l10n-exchange-addon-mailchimp' ),
			'description'       => __( 'Add MailChimp Opt-In Checkbox to user registration form.', 'it-l10n-exchange-addon-mailchimp' ),
			'author'            => 'iThemes',
...
function it_exchange_add_mailchimp_nag() {
	...
	<div id="it-exchange-mailchimp-nag" class="it-exchange-nag">
		<?php
		printf( __( 'To use the MailChimp add-on for iThemes Exchange, you must be using iThemes Exchange version 1.0.3 or higher. <a href="%s">Please update now</a>.', 'it-l10n-exchange-addon-mailchimp' ), admin_url( 'update-core.php' ) );
		?>
	</div>
    <?php
}
...
function it_exchange_mailchimp_set_textdomain() {
	load_plugin_textdomain( 'it-l10n-exchange-addon-mailchimp', false, dirname( plugin_basename( __FILE__  ) ) . '/lang/' );
}
