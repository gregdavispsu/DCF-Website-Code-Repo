...
class IT_Exchange_Product_Feature_MailChimp {
	...
	*/
	function add_feature_support_to_product_types() {
		...
		$slug        = 'mailchimp';
		$description = __( "Set the Product's MailChimp options", 'it-l10n-exchange-addon-mailchimp' );
	...
	*/
	function register_metabox() {
		add_meta_box( 'it-exchange-product-mailchimp', __( 'MailChimp', 'it-l10n-exchange-addon-mailchimp' ), array( $this, 'print_metabox' ), 'it_exchange_prod', 'normal' );
	}
	...
	*/
	function print_metabox( $product ) {
		...
		echo '<p>' . __( 'When a customer purchases this product, add them to this list...', 'it-l10n-exchange-addon-mailchimp' ) . '</p>';
		if ( !empty( $settings['mailchimp-api-key'] ) ) {
			...
			?>
			<h4><label for="mailchimp-list"><?php _e( 'MailChimp List', 'it-l10n-exchange-addon-mailchimp' ) ?> <span class="tip" title="<?php _e( 'This is the list you want to use from MailChimp.', 'it-l10n-exchange-addon-mailchimp' ); ?>">i</span></label></h4>
			<select name="it-exchange-add-on-mailchimp-list-id">
				<option value="0"><?php _e( 'Select a Mailchimp List', 'it-l10n-exchange-addon-mailchimp' ); ?></option>
				<?php
			...
			<?php
				...
				_e( 'It appears that you do not have any MailChimp lists setup, go to Mailchimp and add at least one list to setup Mailchimp for this product.', 'it-l10n-exchange-addon-mailchimp' );
			}
		} else {
			_e( 'You have not added a Mailchimp API key to your addon settings, please go to the Mailchimp add-on settings to complete the setup and return to this product to finish setting it up with a Mailchimp list.', 'it-l10n-exchange-addon-mailchimp' );
		}
