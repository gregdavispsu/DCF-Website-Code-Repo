...
function it_exchange_addon_recurring_payments_show_version_nag() {
	if ( version_compare( $GLOBALS['it_exchange']['version'], '1.3.0', '<' ) ) {
		...
		<div id="it-exchange-add-on-min-version-nag" class="it-exchange-nag">
			<?php printf( __( 'The Recurring Payments add-on requires iThemes Exchange version 1.3.0 or greater. %sPlease upgrade Exchange%s.', 'it-l10n-exchange-addon-recurring-payments' ), '<a href="' . admin_url( 'update-core.php' ) . '">', '</a>' ); ?>
		</div>
...
function it_exchange_recurring_payments_addon_recurring_label( $product_id ) {
	...
	if ( it_exchange_product_has_feature( $product_id, 'recurring-payments', array( 'setting' => 'recurring-enabled' ) ) ) {
		if ( it_exchange_get_product_feature( $product_id, 'recurring-payments', array( 'setting' => 'recurring-enabled' ) ) ) {
			...
			if ( 1 == $interval_count ) {
				$label .= '&nbsp;' . sprintf( __( 'every %s', 'it-l10n-exchange-addon-recurring-payments' ), it_exchange_recurring_payments_addon_interval_string( $interval, $interval_count ) );
			} else if ( 0 < $interval_count ) {
				$label .= '&nbsp;' . sprintf( __( 'every %s %s', 'it-l10n-exchange-addon-recurring-payments' ), $interval_count, it_exchange_recurring_payments_addon_interval_string( $interval, $interval_count ) );
			}
			...
			if ( $trial_enabled ) {	
				...
				if ( $show_trial && 0 < $trial_interval_count ) {
					$label .= '&nbsp;' . sprintf( __( '(after %s %s free)', 'it-l10n-exchange-addon-recurring-payments' ), $trial_interval_count, it_exchange_recurring_payments_addon_interval_string( $trial_interval, $trial_interval_count ) );
				}
			}
