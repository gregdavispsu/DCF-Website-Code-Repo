	...
	if ( is_plugin_active( 'exchange-addon-membership/exchange-addon-membership.php' ) ) {
		if ( !function_exists( 'it_exchange_membership_addon_get_all_the_children' ) ) {
			...
			<div id="it-exchange-add-on-min-version-nag" class="it-exchange-nag">
				<?php printf( __( 'Your version of the Membership add-on is not compatible with your Recurring Payments add-on. Please update to version 1.2.16 or greater. %sClick here to upgrade the Membership add-on%s.', 'it-l10n-exchange-addon-recurring-payments' ), '<a href="' . admin_url( 'update-core.php' ) . '">', '</a>' ); ?>
			</div>
...
function it_exchange_recurring_payments_transaction_print_metabox_after_product_feature_title( $post, $product ) {
	...
	if ( empty( $enabled ) ) {
		$time = __( 'forever', 'it-l10n-exchange-addon-recurring-payments' );
			...
			if ( it_exchange_get_product_feature( $product['product_id'], 'recurring-payments', array( 'setting' => 'recurring-enabled' ) ) ) {
				...
				<div class="transaction-recurring-options clearfix spacing-wrapper">
					<div class="recurring-options left">
						<h3><?php _e( 'Subscription Settings', 'it-l10n-exchange-addon-recurring-payments' ); ?></h3>
						<form action="" method="POST">
							...
							?>
							<p>
							<label for="recurring-payment-subscriber-id"><?php _e( 'Subscription ID', 'it-l10n-exchange-addon-recurring-payments' ); ?> <span class="tip" title="<?php _e( 'This is the Subscription ID from the Payment Processor.', 'it-l10n-exchange-addon-recurring-payments' ); ?>">i</span></label>
							...
							<p>
							<label for="recurring-payment-subscriber-status"><?php _e( 'Subscription Status', 'it-l10n-exchange-addon-recurring-payments' ); ?> <span class="tip" title="<?php _e( 'This is the status of the subscription in Exchange, not the transaction. It will not change the status in the Payment gateway.', 'it-l10n-exchange-addon-recurring-payments' ); ?>">i</span></label>
							<select id="recurring-payment-subscriber-status" name="recurring-payment-subscriber-status">
								<option value="active" <?php selected( 'active', $subscriber_status, true ); ?>><?php _e( 'Active', 'it-l10n-exchange-addon-recurring-payments' ); ?></option>
								<option value="suspended" <?php selected( 'suspended', $subscriber_status, true ); ?>><?php _e( 'Suspended', 'it-l10n-exchange-addon-recurring-payments' ); ?></option>
								<option value="cancelled" <?php selected( 'cancelled', $subscriber_status, true ); ?>><?php _e( 'Cancelled', 'it-l10n-exchange-addon-recurring-payments' ); ?></option>
								<option value="deactivated" <?php selected( 'deactivated', $subscriber_status, true ); ?>><?php _e( 'Deactivated', 'it-l10n-exchange-addon-recurring-payments' ); ?></option>
							</select>
							</p>
							
							<p>
							<label for="recurring-payment-subscriber-expires"><?php _e( 'Subscription Expiration', 'it-l10n-exchange-addon-recurring-payments' ); ?> <span class="tip" title="<?php _e( 'Set this to change what Exchange sees as the customer expiration date, the Payment processor will still send webhooks if the payment expires or if new payments come through.', 'it-l10n-exchange-addon-recurring-payments' ); ?>">i</span></label>
							...
							<p class="description">
							<?php _e( "Warning:  Changes to these settings can potentially remove this customer's access to their products.", 'it-l10n-exchange-addon-recurring-payments' ); ?>
							</p>
...
function it_exchange_recurring_payments_save_transaction_post( $post_id, $post, $update ) {
	...
	if ( !empty( $_POST['transaction-recurring-options-nonce'] ) ) {
		...
		if ( wp_verify_nonce( $_POST['transaction-recurring-options-nonce'], 'transaction-recurring-options' ) ) {
            ...
            if ( $wp_error ) {
                return new WP_Error( 'invalid_nonce', __( 'Unable to verify security none.', 'it-l10n-exchange-addon-recurring-payments' ) );
            }
            return 0;
