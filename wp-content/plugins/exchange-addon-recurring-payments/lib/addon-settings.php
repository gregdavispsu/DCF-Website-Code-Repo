...
function it_exchange_recurring_payments_addon_default_settings( $values ) {
    $defaults = array(
        'recurring-payments-cancel-subject'   => __( 'Cancellation Notification', 'it-l10n-exchange-addon-recurring-payments' ),
        'recurring-payments-cancel-body'      => __( 'Hello [it_exchange_email show=name],

Your recurring payment has been cancelled.

Thank you.
[it_exchange_email show=sitename]', 'it-l10n-exchange-addon-recurring-payments' ),
        'recurring-payments-deactivate-subject'   => __( 'Expiration Notification', 'it-l10n-exchange-addon-recurring-payments' ),
    ...
    */
    function print_settings_page() {
        ...
        <div class="wrap">
            ...
            <h2><?php _e( 'Recurring Payments Settings', 'it-l10n-exchange-addon-recurring-payments' ); ?></h2>
            ...
            <?php $form->start_form( $form_options, 'it-exchange-recurring-payments-settings' ); ?>
                ...
                <p class="submit">
                    <?php $form->add_submit( 'submit', array( 'value' => __( 'Save Changes', 'it-l10n-exchange-addon-recurring-payments' ), 'class' => 'button button-primary button-large' ) ); ?>
                </p>
            <?php $form->end_form(); ?>
    ...
    function get_recurring_payments_form_table( $form, $settings = array() ) {
        ...
        if ( ! empty( $_GET['page'] ) && 'it-exchange-setup' == $_GET['page'] ) : ?>
            <h3><?php _e( 'Recurring Payments', 'it-l10n-exchange-addon-recurring-payments' ); ?></h3>
        <?php endif; ?>
        <div class="it-exchange-addon-settings it-exchange-recurring-payments-addon-settings">
            <h4><?php _e( 'Recurring Payment Cancelled Email', 'it-l10n-exchange-addon-recurring-payments' ); ?></h4>
            <p>
                <label for="recurring-payments-cancel-subject"><?php _e( 'Email Subject', 'it-l10n-exchange-addon-recurring-payments' ); ?> <span class="tip" title="<?php _e( 'The subject you want users who have cancelled their subscriptions to receive.', 'it-l10n-exchange-addon-recurring-payments' ); ?>">i</span></label>
                <?php $form->add_text_box( 'recurring-payments-cancel-subject' ); ?>
            </p>
            <p>
                <label for="recurring-payments-cancel-body"><?php _e( 'Email Message', 'it-l10n-exchange-addon-recurring-payments' ); ?> <span class="tip" title="<?php _e( 'The message you want users who have cancelled their subscriptions to receive.', 'it-l10n-exchange-addon-recurring-payments' ); ?>">i</span></label>
                <?php
            ...
            <h4><?php _e( 'Recurring Payment Expired Email', 'it-l10n-exchange-addon-recurring-payments' ); ?></h4>
            <p>
                <label for="recurring-payments-deactivate-subject"><?php _e( 'Email Subject', 'it-l10n-exchange-addon-recurring-payments' ); ?> <span class="tip" title="<?php _e( 'The subject you want users who have cancelled their subscriptions to receive.', 'it-l10n-exchange-addon-recurring-payments' ); ?>">i</span></label>
                <?php $form->add_text_box( 'recurring-payments-deactivate-subject' ); ?>
            </p>
            <p>
                <label for="recurring-payments-deactivate-body"><?php _e( 'Email Message', 'it-l10n-exchange-addon-recurring-payments' ); ?> <span class="tip" title="<?php _e( 'The message you want users who have cancelled their subscriptions to receive.', 'it-l10n-exchange-addon-recurring-payments' ); ?>">i</span></label>
                <?php
                if ( $wp_version >= 3.3 && function_exists( 'wp_editor' ) ) {
            ...
            <p class="description">
            <?php 
            _e( 'Enter the email that is sent to administrator after a customer completes a successful purchase. HTML is accepted. Available shortcode functions:', 'it-l10n-exchange-addon-recurring-payments' ); 
            echo '<br />';
            printf( __( 'You call these shortcode functions like this: %s', 'it-l10n-exchange-addon-recurring-payments' ), '[it_exchange_email show=order_table option=purchase_message]' ); 
            echo '<ul>';
            echo '<li>name - ' . __( "The buyer's first name", 'it-l10n-exchange-addon-recurring-payments' ) . '</li>';
            echo '<li>fullname - ' . __( "The buyer's full name, first and last", 'it-l10n-exchange-addon-recurring-payments' ) . '</li>';
            echo '<li>username - ' . __( "The buyer's username on the site, if they registered an account", 'it-l10n-exchange-addon-recurring-payments' ) . '</li>';
            echo '<li>sitename - ' . __( 'Your site name', 'it-l10n-exchange-addon-recurring-payments' ) . '</li>';
            echo '<li>login_link - ' . __( 'Adds a link to the login page on your website.', 'it-l10n-exchange-addon-recurring-payments' ) . '</li>';
    ...
    */
    function save_settings() {
        ...
        if ( ! wp_verify_nonce( $_POST['_wpnonce'], 'it-exchange-recurring-payments-settings' ) ) {
            $this->error_message = __( 'Error. Please try again', 'it-l10n-exchange-addon-recurring-payments' );
            return;
        }
        ...
        if ( ! $errors && it_exchange_save_option( 'addon_recurring_payments', $new_values ) ) {
            ITUtility::show_status_message( __( 'Settings saved.', 'it-l10n-exchange-addon-recurring-payments' ) );
        } else if ( $errors ) {
            $errors = implode( '<br />', $errors );
            $this->error_message = $errors;
        } else {
            $this->status_message = __( 'Settings not saved.', 'it-l10n-exchange-addon-recurring-payments' );
        }
    }
    ...
    */
    public function get_form_errors( $values ) {
        ...
        if ( empty( $values['recurring-payments-cancel-subject'] ) )
            $errors[] = __( 'Please include an email subject for cancellations', 'it-l10n-exchange-addon-recurring-payments' );
        if ( empty( $values['recurring-payments-cancel-body'] ) )
            $errors[] = __( 'Please include an email body for cancellations', 'it-l10n-exchange-addon-recurring-payments' );
        if ( empty( $values['recurring-payments-deactivate-subject'] ) )
            $errors[] = __( 'Please include an email subject for expirations', 'it-l10n-exchange-addon-recurring-payments' );
        if ( empty( $values['recurring-payments-deactivate-body'] ) )
            $errors[] = __( 'Please include an email body for expirations', 'it-l10n-exchange-addon-recurring-payments' );
