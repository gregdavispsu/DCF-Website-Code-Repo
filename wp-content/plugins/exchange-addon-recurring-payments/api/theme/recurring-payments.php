	...
	*/
	function unsubscribe( $options=array() ) {
		$defaults = array(
			...
			'label'  => apply_filters( 'it_exchange_recurring_payments_addon_unsubscribe_label', __( 'Cancel this subscription', 'it-l10n-exchange-addon-recurring-payments' ) ),
		);
		...
		if ( it_exchange_get_recurring_payments_addon_transaction_subscription_id( $this->_transaction ) ) {
			...
			switch( $subscription_status ) {
				case 'deactivated' :
					$output .= __( 'Subscription deactivated', 'it-l10n-exchange-addon-recurring-payments' );
					break;
				case 'cancelled' :
					$output .= __( 'Subscription cancelled', 'it-l10n-exchange-addon-recurring-payments' );
					break;
				case 'suspended' :
					$output .= __( 'Subscription suspended', 'it-l10n-exchange-addon-recurring-payments' );
					break;
				case 'active' :
				default:
	...
	*/
	function expiration( $options=array() ) {
		$defaults = array(
			...
			'label'            => apply_filters( 'it_exchange_recurring_payments_addon_expiration_label', __( 'Expires', 'it-l10n-exchange-addon-recurring-payments' ) ),
	...
	*/
	function payments( $options=array() ) {
		$defaults = array(
			...
			'label'            => apply_filters( 'it_exchange_recurring_payments_addon_payments_label', __( 'Payment', 'it-l10n-exchange-addon-recurring-payments' ) ),
		);
		$options = ITUtility::merge_defaults( $options, $defaults );
		$output = '';
		if ( $this->_transaction->has_children() ) {
			...
			foreach ( $payment_transactions as $transaction ) {
				$output .= '<li>';
				$output .= $options['label'] . ' ' . __( 'of', 'it-l10n-exchange-addon-recurring-payments' ) . ' ' . it_exchange_get_transaction_total( $transaction, $options['format_currency'] ) . ' on ' . it_exchange_get_transaction_date( $transaction, $options['date_format'] ) . ' - ' . it_exchange_get_transaction_status_label( $transaction );
			}
			$output .= '</ul>';
			$output .= $options['after'];
		}
