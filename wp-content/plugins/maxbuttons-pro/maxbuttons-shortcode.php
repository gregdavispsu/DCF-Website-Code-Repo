...
function mbpro_button_shortcode($atts) {
	...
	if ($button_id != '' || $button_name != '') {
		if($button_name != '') {
			$button = mbpro_get_button($button_name);
		} else {
		...
		if (isset($button)) {
			...
			$show_fa_icon = false;
			if ($button->use_font_awesome_icon == 'on') {
				$show_fa_icon = true;
				$show_icon = false;
			}
			...
			$css .= 'display: block;'; // this is a fix for ios / safari 
			...
			if ($show_icon && $show_fa_icon != 'on') {
				...
				$css .= 'width: ' . $icon_width . $important . 'px; ';
				$css .= '} ';
				...
				// Check how to handle the icon width
				$icon_width = $button->font_awesome_icon_size;
				
				$css .= 'a.maxbutton-' . $button->id . ' .maxbutton .mb-icon { ';
				$css .= 'font-size: ' . $button->font_awesome_icon_size . 'px ' . $important . '; ';
				$css .= 'line-height: ' . $button->font_awesome_icon_size . 'px ' . $important . '; ';
				$css .= 'padding-top: ' . $button->icon_padding_top . $important . '; ';
				$css .= 'padding-right: ' . $button->icon_padding_right . $important . '; ';
				$css .= 'padding-bottom: ' . $button->icon_padding_bottom . $important . '; ';
				$css .= 'padding-left: ' . $button->icon_padding_left . $important . '; ';
				...
				$css .= '} ';
				$css .= 'a.maxbutton-' . $button->id . ' .maxbutton .mb-icon i { ';
			...
			if ($button_url != '') {
				...
				if ($show_fa_icon) {
					$css .= 'a.maxbutton-' . $button->id . ':hover .maxbutton .mb-icon i { ';
					$css .= 'color: ' . $button->icon_color_hover . '; ';
					$css .= '} ';
				}
			}
			...
			if ($button_url == '') {
				...
				$output .= '<a class="maxbutton-' . $button->id . '" href="' . do_shortcode($button_url) . '" ' . $button_window . ' ' . $button_nofollow . '>';
			}
			...
			} elseif (!$show_icon && $show_fa_icon ) {
				if ($button->icon_position == 'left') {
					$output .= '<div class="mb-icon mb-left"><i class="fa ' . $button->font_awesome_icon . '"></i></div>';
				}

				if ($button->icon_position == 'right') {
					$output .= '<div class="mb-icon mb-right"><i class="fa ' . $button->font_awesome_icon . '"></i></div>';
				}
				
				if ($button->icon_position == 'top') {
					$output .= '<div class="mb-icon mb-top"><i class="fa ' . $button->font_awesome_icon . '"></i></div>';
				}
				
			}
			
			if ($button_text != '') {
				$output .= '<div class="mb-text">' . do_shortcode($button_text) . '</div>';
			}
			
			if ($button_text2 != '') {
				$output .= '<div class="mb-text2">' . do_shortcode($button_text2) . '</div>';
			}

			if ($show_icon && $button->icon_position == 'bottom') {
				$output .= '<div class="mb-icon mb-bottom"><img src="' . do_shortcode($button->icon_url) . '" alt="' . $button->icon_alt . '" border="0" /></div>';
			}

			if (!$show_icon && $show_fa_icon && $button->icon_position == 'bottom') {
				$output .= '<div class="mb-icon mb-bottom"><i class="fa ' . $button->font_awesome_icon . '"></i></div>';
			}

			if($show_icon || $show_fa_icon) {
