...
function mbpro_set_global_paths() {
	...
	define('MAXBUTTONS_PRO_VERSION_NUM', '2.12');
	...
	define('MAXBUTTONS_PRO_CAPABILITIES', 'maxbuttons_pro_user_level');
}
...
add_action( 'admin_init', 'mbpro_register_settings' );
function mbpro_register_settings() {
	register_setting( 'mbpro_settings', 'maxbuttons_pro_user_level' );
}

add_action('admin_menu', 'mbpro_admin_menu');
function mbpro_admin_menu() {
	$mbpro_capabilities = get_option('maxbuttons_pro_user_level');
	if(!$mbpro_capabilities) {
		$mbpro_capabilities = 'manage_options';
		settings_fields( 'mbpro_settings' );
		update_option('maxbuttons_pro_user_level', $mbpro_capabilities);
	}
	$admin_pages = array();
	
	$page_title = __('MaxButtons Pro : Buttons', 'maxbuttons-pro');
	$menu_title = __('Buttons', 'maxbuttons-pro');
	$capability = $mbpro_capabilities;
...
function mbpro_add_admin_styles() {		
	wp_register_style('mbpro-font-awesome', '//maxcdn.bootstrapcdn.com/font-awesome/4.2.0/css/font-awesome.min.css', '', '4.2' );
...
function mbpro_media_button($context) {
	...
	if (in_array($pagenow, array('post.php', 'page.php', 'post-new.php', 'post-edit.php'))) {
		...
		$output = '<a href="#TB_inline?width=800&inlineId=select-maxbutton-container" class="thickbox button" title="' . $title . '" style="padding-left: .4em;">' . $img . ' ' . $title . '</a>';
	}
...
function mbpro_create_database_table() {
	...
	$sql = "CREATE TABLE " . $table_name . " (
				...
				icon_color_hover VARCHAR(10) NULL,
				...
				use_font_awesome_icon VARCHAR(40) NULL,
				font_awesome_icon VARCHAR(40) NULL,
				icon_url VARCHAR(250) NULL,
				icon_position VARCHAR(10) NULL,
				font_awesome_icon_size VARCHAR(10) NULL,
...
function mbpro_get_button($id) {
	...
	if(!is_numeric($id)) {
		return $wpdb->get_row($wpdb->prepare("SELECT * FROM " . mbpro_get_buttons_table_name() . " WHERE name = '".$id."'", $id));
	} else {
...
function mbpro_get_published_buttons() {
	...
	return $wpdb->get_results(
		$wpdb->prepare("SELECT * FROM " . mbpro_get_buttons_table_name() . " WHERE status <> 'trash' %s ",$order));
}
