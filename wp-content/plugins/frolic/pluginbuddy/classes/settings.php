...
class pb_frolic_settings {
	 ...
	 *	@param		string/false	$save_point_or_custom_mode	Location in pb_frolic::$options array to save to. Ex: groups#5 saves into: pb_frolic::$options['groups'][5].
	...
	function __construct( $form_name, $save_point_or_custom_mode, $additional_query_string = '', $custom_title_width = '' ) {
		...
		$this->_form = new pb_frolic_form( $form_name, $save_point_or_custom_mode, $additional_query_string );
	} // End __construct().
	...
	function add_setting( $settings ) {
		...
		if ( $settings['default'] != '' ) { // Default was passed to add_setting().
			...
			if ( $this->_savepoint !== false ) {
				...
				$group = pb_frolic::get_group( $this->_savepoint );
				if ( $group === false ) {
					$default_value = '';
				} else {
	...
	public function process() {
		if ( ( '' != ( $form_name = pb_frolic::_POST( $this->_prefix ) ) ) && ( pb_frolic::_POST( $this->_prefix ) == $this->_form_name ) ) { // This form was indeed submitted. PROCESS IT!
			// TODO:
			$errors = array();
			$_posts = pb_frolic::_POST();
			...
			if ( count( $errors ) > 0 ) { // Errors.
				pb_frolic::alert( 'Error validating one or more fields as indicated below. Error(s):<br>' . implode( '<br>', $errors ), true );
			}
			...
			if ( $this->_savepoint === false ) {
				...
				foreach( $_posts as $post_name => $post_value ) { // Loop through all post items (not all may be our form). @see 83429594837534987.
					//if ( substr( $post_name, 0, strlen( $this->_prefix ) ) == $this->_prefix ) { // If prefix matches, its this form.
						...
						//if ( $post_name != '' ) { // Skips the empty prefix name we used for verifying form. @see 83429594837534987.
							...
							$savepoint_subsection = &pb_frolic::$options;
				...
				pb_frolic::save();
				pb_frolic::alert( __( 'Settings saved.' . $error_note, 'it-l10n-frolic' ) );
				//} // end no errors.
	...
	public function get_settings( $submit_button_title ) {
		...
		foreach ( $this->_settings as $settings ) {
			...
			if ( $settings['type'] == 'title' ) { // Title item.
				...
				$return .= $this->_form->get( $settings['name'], $settings['css'] );
			} else { // Normal item.
				$return .= '<tr>';
				$return .= '<th scope="row"';
				...
				if ( isset( $settings['tip'] ) && ( $settings['tip'] != '' ) ) {
					$return .= pb_frolic::$ui->tip( $settings['tip'], '', false );
				}
				$return .= '</th>';
				...
				$return .= $this->_form->get( $settings['name'], $settings['css'], $settings['orientation'] );
	...
	$return .= '<br>';
		...
		$return .= $this->_form->get( 'settings_submit', $settings['name'] );
