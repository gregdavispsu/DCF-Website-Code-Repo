...
class pb_frolic_form {
	...
	function __construct( $form_name, $save_point = '', $additional_query_string = '' ) {
		...
		$this->_prefix = 'pb_' . pb_frolic::settings( 'slug' ) . '_';
	} // End __construct().
	...
	public function start() {
		$this->_started = true;
		$return = '<form method="post" action="' . pb_frolic::page_url() . '&' . $this->_additional_query_string . '" id="' . $this->_prefix . $this->_form_name . '_form">';
	...
	public function end( $echo = false ) {
		...
		// TODO: fields maybe?
		//$return = pb_frolic::nonce( $this->_form_name );
		$return = pb_frolic::nonce( false ); // Do not echo.
		$return .= '</form>';
	 ...
	 */
	public function get( $name, $css = '', $orientation = 'horizontal' ) {
		...
		if ( isset( $this->_inputs[$name] ) ) {
			...
			if ( $input['type'] == 'text' ) {
				...
				$return .= '<input type="text" name="' . $prefix . $name . '" value="' . $input['value'] . '" id="' . $prefix . $name . '"' . $css . '>';
			...
			/********** COLOR **********/
				...
				if ( $this->_loaded_color === false ) { // Only load the javascript, CSS, etc once per instance.
					pb_frolic::load_script( 'jquery.miniColors.min.js', true );
					pb_frolic::load_style( 'jquery.miniColors.css', true );
					...
					<style type="text/css">
						.miniColors-trigger {
							background: url( ' . pb_frolic::plugin_url() . '/pluginbuddy/images/minicolors/trigger.png ) center no-repeat;
						}
						.miniColors-colors {
							background: url( ' . pb_frolic::plugin_url() . '/pluginbuddy/images/minicolors/gradient.png ) center no-repeat;
						}
						.miniColors-hues {
							background: url( ' . pb_frolic::plugin_url() . '/pluginbuddy/images/minicolors/rainbow.png ) center no-repeat;
						}
						.miniColors-colorPicker {
							background: url( ' . pb_frolic::plugin_url() . '/pluginbuddy/images/minicolors/circle.gif ) center no-repeat;
						}
						.miniColors-huePicker {
							background: url( ' . pb_frolic::plugin_url() . '/pluginbuddy/images/minicolors/line.gif ) center no-repeat;
						}
					</style>';
				...
				$return .= '<input class="pb_colorpicker" type="text" name="' . $prefix . $name . '" value="' . $input['value'] . '" id="' . $prefix . $name . '"' . $css . '>';
			...
			/********** HIDDEN **********/
				...
				$return .= '<input type="hidden" name="' . $prefix . $name . '" value="' . $input['value'] . '" id="' . $prefix . $name . '"' . $css . '>';
			...
			/********** TEXTAREA **********/
				...
				$return .= '<textarea name="' . $prefix . $name . '" id="' . $prefix . $name . '"' . $css . '>' . $input['value'] . '</textarea>';
			...
			/********** PASSWORD **********/
				...
				$return .= '<input type="password" name="' . $prefix . $name . '" value="' . $input['value'] . '" id="' . $prefix . $name . '"' . $css . '>';
			...
			/********** SELECT **********/
				...
				$return .= '<select name="' . $prefix . $name . '" id="' . $prefix . $name . '"' . $css . '>';
			...
			/********** RADIO **********/
				...
				foreach ( $input['options'] as $option_value => $option_title ) {
					$i++;
					$return .= '<input type="radio" name="' . $prefix . $name . '" id="' . $prefix . $name . '" value="' . $option_value . '"' . $css;
			...
			/********** CHECKBOX **********/
				...
				$return .= '<input type="checkbox" name="' . $prefix . $name . '" id="' . $prefix . $name . '" value="' . $input['options']['checked'] . '"' . $css;
			...
			/********** SUBMIT **********/
				...
				$return .= '<input class="button-primary" type="submit" name="' . $prefix . $name . '" value="' . $input['value'] . '" id="' . $prefix . $name . '"' . $css . '>';
	...
	function get_value( $name ) {
		if ( pb_frolic::_POST( $this->_prefix . $name ) != '' ) { // Submitted value exists, use it.
			return stripslashes_deep( pb_frolic::_POST( $this->_prefix . $name ) );
	...
	public function test( $name, $value = '' ) {
		if ( $value == '' ) { // No value, try to get it.
			if ( pb_frolic::_POST( $this->_prefix . $name ) != '' ) { // Submitted value exists, use it.
				$value = pb_frolic::_POST( $this->_prefix . $name );
			//} else { // Nothing submitted, fail.
			//	return false;
			}
		}
