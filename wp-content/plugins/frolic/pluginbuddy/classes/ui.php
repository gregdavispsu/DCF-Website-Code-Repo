...
class pb_frolic_ui {
	...
	public function title( $title, $echo = true ) {
		$return = '<h2><img src="' . pb_frolic::plugin_url() . '/images/icon_32x32.png" style="vertical-align: -7px;"> ' . $title . '</h2><br />';
		if ( $echo === true ) {
			echo $return;
		} else {
			return $return;
	...
	public static function note( $text, $echo = true ) {
		$return = '<span style="color: #AFAFAF;"><i>' . $text . '</i></span>';
		if ( $echo === true ) {
			echo $return;
		} else {
			return $return;
	...
	public static function list_table( $items, $settings ) {
		...
		function bulk_actions( $settings, $hover_note = false ) {
			if ( count( $settings['bulk_actions'] ) > 0 ) {
				...
				if ( $hover_note === true ) {
					echo pb_frolic::$ui->note( 'Hover over items above for additional options.' );
				}
				if ( $settings['reorder'] != '' ) {
		...
		echo '<div style="width: 70%; min-width: 750px; ' . $settings['css'] . '">';
		...
		foreach ( $items as $item_id => $item ) {
			...
			foreach ( $settings['hover_actions'] as $action_slug => $action_title ) { // Display all hover actions.
				$i++;
				echo '<a href="' . pb_frolic::page_url() . '&' . $action_slug . '=' . $item_id . '">' . $action_title . '</a>';
			...
			for ( $i = 1; $i < $count; $i++ ) {
				...
				if ( ( $settings['reorder'] != '' ) && ( $i == ( $settings['reorder'] ) ) ) {
					echo '<img src="' . pb_frolic::plugin_url() . '/pluginbuddy/images/draghandle.png" alt="Click and drag to reorder">';
				} else {
					echo $item[$i];
				}
	...
	public function get_feed( $feed, $limit, $append = '', $replace = '' ) {
		...
		if (!is_wp_error( $rss ) ) {
			...
			$return .= '<ul class="pluginbuddy-nodecor">';
	...
	public function tip( $message, $title = '', $echo_tip = true ) {
		$tip = ' <a class="pluginbuddy_tip" title="' . $title . ' - ' . $message . '"><img src="' . pb_frolic::plugin_url() . '/pluginbuddy/images/pluginbuddy_tip.png" alt="(?)" /></a>';
		if ( $echo_tip === true ) {
			echo $tip;
		} else {
			return $tip;
	...
	public function alert( $message, $error = false, $error_code = '' ) {
		...
		if ( $log_error === true ) {
			pb_frolic::log( $message . ' Error Code: ' . $error_code, 'error' );
		}
	...
	public function video( $video_key, $title = '', $echo_tip = true ) {
		...
		$tip = '<a href="http://www.youtube.com/embed/' . $video_key . '?autoplay=1&TB_iframe=1&width=640&height=400" class="thickbox pluginbuddy_tip" title="Video Tutorial - ' . $title . '"><img src="' . pb_frolic::plugin_url() . '/pluginbuddy/images/pluginbuddy_play.png" alt="(video)" /></a>';
		if ( $echo_tip === true ) {
			echo $tip;
		} else {
			return $tip;
	...
	*/
	...
	public function start_tabs( $interface_tag, $tabs, $css = '', $echo = true ) {
		...
		$prefix = 'pb_' . pb_frolic::settings( 'slug' ) . '_'; // pb_PLUGINSLUG_
		$return = '';
		...
		$return .= '</ul>';
		$return .= '<br>';
		$return .= '<div class="tabs-borderwrap" id="editbox" style="position: relative; height: 100%; -moz-border-radius-topleft: 0px; -webkit-border-top-left-radius: 0px;">';
