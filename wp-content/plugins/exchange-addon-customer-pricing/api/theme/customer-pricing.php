	...
	function base_price( $options=array() ) {
		...
		if ( it_exchange_product_supports_feature( $this->product->ID, 'customer-pricing', array( 'setting' => 'enabled' ) )
			...
			$defaults   = array(
				...
				'after'         => '</span>',
				'format'        => 'html',
				'free-label'    => __( 'Free', 'it-l10n-exchange-addon-customer-pricing' ),
				'show-or-more'  => true,
		...
		if ( it_exchange_product_supports_feature( $this->product->ID, 'customer-pricing', array( 'setting' => 'enabled' ) )
			...
			if ( 'yes' === $nyop_enabled ) {
				...
				if ( !empty( $nyop_min ) && 0 < $nyop_min )
					$result .= '<span class="it-exchange-customer-price-min"><small>' . sprintf( __( 'Min: %s', 'it-l10n-exchange-addon-customer-pricing' ), it_exchange_format_price( it_exchange_convert_from_database_number( $nyop_min ) ) ) . '</small></span>';
					
				if ( !empty( $nyop_max ) && 0 < $nyop_max )
					$result .= '<span class="it-exchange-customer-price-max"><small>' . sprintf( __( 'Max: %s', 'it-l10n-exchange-addon-customer-pricing' ), it_exchange_format_price( it_exchange_convert_from_database_number( $nyop_max ) ) ) . '</small></span>';
				$result .= '</p>';
				$result .= '</div>';
			}
