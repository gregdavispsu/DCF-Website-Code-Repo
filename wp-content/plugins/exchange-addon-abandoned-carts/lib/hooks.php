...
function it_exchange_abandoned_carts_add_admin_menu_item() {
	...
	if( ! empty( $_GET['page'] ) && 'it-exchange-abandoned-carts-settings' == $_GET['page'] )
		add_submenu_page( 'it-exchange', __( 'Abandoned Carts Settings', 'it-l10n-exchange-addon-abandoned-carts' ), __( 'Abandoned Carts', 'it-l10n-exchange-addon-abandoned-carts' ), $admin_menu_capability, 'it-exchange-abandoned-carts-settings', 'it_exchange_abandoned_carts_print_settings_page' );
	else
	*/
	add_submenu_page( 'it-exchange', __( 'Abandoned Carts Dashboard', 'it-l10n-exchange-addon-abandoned-carts' ), __( 'Abandoned Carts', 'it-l10n-exchange-addon-abandoned-carts' ), $admin_menu_capability, 'it-exchange-abandoned-carts-dashboard', 'it_exchange_abandoned_carts_print_dashboard_page' );
}
...
function it_exchange_abandoned_carts_print_dashboard_page() {
	?>
	<div class="wrap">
		<h2><?php _e( 'Abandoned Carts Dashboad', 'it-l10n-exchange-addon-abandoned-carts' ); ?></h2>
		<?php do_action( 'views_edit-it_ex_abandond_carts_dashboard', false ); ?>

		<div class="overview-chart clear">
			<h3><?php _e( 'Recovered Carts', 'it-l10n-exchange-addon-abandoned-carts' ); ?></h3>
			<div class="no-recovered-carts hidden">
				...
				if ( empty( $num_abandoned_carts ) ) :
					_e( 'You haven\'t had any abandoned carts yet. Congratulations!', 'it-l10n-exchange-addon-abandoned-carts' );
				else :
					printf( __( 'You have %s abandoned carts but haven\'t recovered any yet. Make sure youre %semails%s are setup correctly!', 'it-l10n-exchange-addon-abandoned-carts' ), $num_abandoned_carts, '<a href="' . admin_url( 'edit.php?post_type=it_ex_abandoned' ) . '">', '</a>' );
				endif;
				?>
				</p>
	...
	*/
	...
	<div class="it-exchange-abandoned-carts-dashboard">
		...
		<div class="abdandoned-carts-nav">
			...
			<a class="nav-tab <?php echo ($current_tab == 'dashboard' ) ? 'nav-tab-active' : '';?>" href="<?php echo admin_url( 'admin.php?page=it-exchange-abandoned-carts-dashboard' ); ?>"><?php _e( 'Dashboard', 'it-l10n-exchange-addon-abandoned-carts' ); ?></a>
			<a class="nav-tab <?php echo ($current_tab == 'carts' ) ? 'nav-tab-active' : '';?>" href="<?php echo admin_url( 'edit.php?post_type=it_ex_abandoned' ); ?>"><?php _e( 'Carts', 'it-l10n-exchange-addon-abandoned-carts' ); ?></a>
			<a class="nav-tab <?php echo ($current_tab == 'emails' ) ? 'nav-tab-active' : '';?>" href="<?php echo admin_url( 'edit.php?post_type=it_ex_abandond_email' ); ?>"><?php _e( 'Email Templates', 'it-l10n-exchange-addon-abandoned-carts' ); ?></a>
			<?php
			/**
			 * Don't need settings yet
			<a class="nav-tab <?php echo ($current_tab == 'settings' ) ? 'nav-tab-active' : '';?>" href="<?php echo admin_url( 'admin.php?page=it-exchange-abandoned-carts-settings' ); ?>"><?php _e( 'Settings', 'it-l10n-exchange-addon-abandoned-carts' ); ?></a>
			*/ ?>
			</h3>
		</div>

	</div>
	<a class="button button-primary it-exchange-abandoned-carts-add-new-email-template hide-if-js" href="<?php echo admin_url('post-new.php?post_type=it_ex_abandond_email'); ?>"><?php _e( 'Add New Email Template', 'it-l10n-exchange-addon-abandoned-carts' ); ?></a>
	<?php
...
function it_exchange_abandoned_carts_add_admin_link_for_all_email_templates() {
	...
	if ( ! empty( $current_screen->id ) && 'it_ex_abandond_email' == $current_screen->id )
		echo '<a class="it-exchange-back-to-all-abandoned-cart-templates h2-add-new hidden" href="' . get_admin_url() . '/edit.php?post_type=it_ex_abandond_email' . '">' . __( '&#8592; Back to all email templates', 'it-l10n-exchange-addon-abandoned-carts' ) . '</a>';
}
...
function it_exchange_abandoned_carts_do_email_shortcodes( $atts=array() ) {
	...
	switch ( $display ) {
		case 'customer_name' :
			$return = empty( $subs['customer_name'] ) ? __( 'Valued Customer', 'it-l10n-exchange-addon-abandoned-carts' ) : $subs['customer_name'];
			break;
		case 'customer_first_name' :
			$return = empty( $subs['customer_first_name'] ) ? __( 'Valued Customer', 'it-l10n-exchange-addon-abandoned-carts' ) : $subs['customer_first_name'];
			break;
...
function it_exchange_abandoned_cart_emails_create_example_email() {
	...
	$args = array(
		...
		'post_status'  => 'draft',
		'post_title'   => __( 'You Forgot Something Awesome!', 'it-l10n-exchange-addon-abandoned-carts' ),
