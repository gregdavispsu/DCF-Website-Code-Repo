...
class IT_Exchange_Abandoned_Cart_Post_Type {
	...
	*/
	...
	function init() {
		...
		$labels    = array(
			...
			'singular_name' => __( 'Abandoned Cart', 'it-l10n-exchange-addon-abandoned-carts' ),
            'search_items'       => __( 'Search Carts', 'it-l10n-exchange-addon-abandoned-carts' ),
            'not_found'          => __( 'No carts found', 'it-l10n-exchange-addon-abandoned-carts' ),
            'not_found_in_trash' => __( 'No carts found in trash', 'it-l10n-exchange-addon-abandoned-carts' ),
		);
		$this->options = array(
			'labels'               => $labels,
			'description'          => __( 'An iThemes Exchange Post Type for storing all Abandoned Carts in the system', 'it-l10n-exchange-addon-abandoned-carts' ),
			'public'               => false,
			'show_ui'              => true,
	...
	*/
	function modify_all_abandoned_carts_table_columns( $existing ) {
		...
		$exchange_columns = array(
			'date'                                          => __( 'Date', 'it-l10n-exchange-addon-abandoned-carts' ),
			'it_exchange_abandoned_cart_customer_column'    => __( 'Customer', 'it-l10n-exchange-addon-abandoned-carts' ),
			'it_exchange_abandoned_cart_status_column'      => __( 'Cart Status', 'it-l10n-exchange-addon-abandoned-carts' ),
			'it_exchange_abandoned_cart_emails_sent_column' => __( 'Emails Sent', 'it-l10n-exchange-addon-abandoned-carts' ),
			'it_exchange_abandoned_cart_total_column'       => __( 'Cart Value', 'it-l10n-exchange-addon-abandoned-carts' ),
		);
		...
		switch( $column ) {
			...
			case 'it_exchange_abandoned_cart_customer_column' :
				if ( $customer = it_exchange_get_customer( $abandoned_cart->customer_id ) )
					...
					esc_attr_e( __( 'Unknown', 'it-l10n-exchange-addon-abandoned-carts' ) );
				break;
			case 'it_exchange_abandoned_cart_emails_sent_column' :
				if ( empty( $abandoned_cart->emails_sent ) || ! is_array( $abandoned_cart->emails_sent ) ) {
					esc_attr_e( __( 'None', 'it-l10n-exchange-addon-abandoned-carts' ) );
					break;
				}
	...
	*/
	function register_abandoned_cart_details_admin_metabox( $post ) {
		...
		// Cart Details
		$title     = __( 'Abandoned Cart Details', 'it-l10n-exchange-addon-abandoned-carts' );
