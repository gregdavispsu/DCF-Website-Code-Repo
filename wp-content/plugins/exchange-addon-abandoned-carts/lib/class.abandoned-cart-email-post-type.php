...
class IT_Exchange_Abandoned_Cart_Email_Post_Type {
	...
	*/
	...
	function init() {
		...
		$labels    = array(
			'name'               => __( 'Abandoned Cart Email Templates', 'it-l10n-exchange-addon-abandoned-carts' ),
			'singular_name'      => __( 'Abandoned Cart Email Template', 'it-l10n-exchange-addon-abandoned-carts' ),
			'add_new_item'       => __( 'Add New Email Template', 'it-l10n-exchange-addon-abandoned-carts' ),
			'edit_item'          => __( 'Edit Email Template', 'it-l10n-exchange-addon-abandoned-carts' ),
			'new_item'           => __( 'New Email Template', 'it-l10n-exchange-addon-abandoned-carts' ),
			'search_items'       => __( 'Search Templates', 'it-l10n-exchange-addon-abandoned-carts' ),
			'not_found'          => __( 'No templates found', 'it-l10n-exchange-addon-abandoned-carts' ),
			'not_found_in_trash' => __( 'No templates found in trash', 'it-l10n-exchange-addon-abandoned-carts' ),
		);
		$this->options = array(
			'labels'               => $labels,
			'description'          => __( 'An iThemes Exchange Post Type for storing all Abandoned Cart email templates in the system', 'it-l10n-exchange-addon-abandoned-carts' ),
			'public'               => false,
			'show_ui'              => true,
	...
	*/
	function modify_all_abandoned_cart_emails_table_columns( $existing ) {
		...
		$exchange_columns = array(
			'cb'                                                   => $check,
			'title'                                                => __( 'Subject', 'it-l10n-exchange-addon-abandoned-carts' ),
			'it_exchange_abandoned_cart_email_status_column'       => __( 'Status', 'it-l10n-exchange-addon-abandoned-carts' ),
			'it_exchange_abandoned_cart_email_scheduling_column'   => __( 'Scheduling', 'it-l10n-exchange-addon-abandoned-carts' ),
			'it_exchange_abandoned_cart_email_delivered_column'    => __( 'Emails Sent', 'it-l10n-exchange-addon-abandoned-carts' ),
			'it_exchange_abandoned_cart_email_opened_column'       => __( 'Opened Rate', 'it-l10n-exchange-addon-abandoned-carts' ),
			'it_exchange_abandoned_cart_email_clickthrough_column' => __( 'Clickthrough Rate', 'it-l10n-exchange-addon-abandoned-carts' ),
			'it_exchange_abandoned_cart_email_recovered_column'    => __( 'Recovered Rate', 'it-l10n-exchange-addon-abandoned-carts' ),
		);
	...
	*/
	function register_abandoned_cart_email_details_admin_metabox( $post ) {
		...
		// Cart Details
		$title     = __( 'Scheduling', 'it-l10n-exchange-addon-abandoned-carts' );
		...
		$title     = __( 'Available Shortcodes', 'it-l10n-exchange-addon-abandoned-carts' );
	...
	*/
	function modify_title_placeholder( $placeholder, $post ) {
		if (  'it_ex_abandond_email' == get_post_type( $post ) )
			$placeholder = __( 'Enter subject here', 'it-l10n-exchange-addon-abandoned-carts' );
	...
	*/
	function print_abandoned_cart_email_scheduling_metabox( $post ) {
		...
		?>
		<p><?php _e( 'How long should we wait to send this email after a customer abandons their cart?', 'it-l10n-exchange-addon-abandoned-carts' ); ?></p>
		...
		<select name="it-exchange-abandonded-cart-emails-scheduling[unit]">
			...
			<option value="minutes" <?php selected( 'minutes', $selected_unit ); ?>><?php _e( 'minutes', 'it-l10n-exchange-addon-abandoned-carts' ); ?></option>
			<?php endif; ?>
			<option value="hours" <?php selected( 'hours', $selected_unit ); ?>><?php _e( 'hour(s)', 'it-l10n-exchange-addon-abandoned-carts' ); ?></option>
			<option value="days" <?php selected( 'days', $selected_unit ); ?>><?php _e( 'day(s)', 'it-l10n-exchange-addon-abandoned-carts' ); ?></option>
			<option value="weeks" <?php selected( 'weeks', $selected_unit ); ?>><?php _e( 'week(s)', 'it-l10n-exchange-addon-abandoned-carts' ); ?></option>
		</select>
	...
	*/
	function print_abandoned_cart_email_shortcodes_metabox( $post ) {
		...
		<hr />
		<p><pre>[exchange-abandoned-carts display="customer_name"]</pre><?php _e( 'Replaced with the customer\'s WordPress display name. Should be first and last name if registered through Exchange', 'it-l10n-exchange-addon-abandoned-carts' ); ?><br /><br /></p><hr />
		<p><pre>[exchange-abandoned-carts display="customer_first_name"]</pre><?php _e( 'Replaced with the customer\'s first name. Uses "Customer" if blank.', 'it-l10n-exchange-addon-abandoned-carts' ); ?><br /><br /></p><hr />
		<p><pre>[exchange-abandoned-carts display="customer_last_name"]</pre><?php _e( 'Replaced with the customer\'s last name if available. Blank if not.', 'it-l10n-exchange-addon-abandoned-carts' ); ?><br /><br /></p><hr />
		<p><pre>[exchange-abandoned-carts display="store_name"]</pre><?php _e( 'Replaced with the store name in General Settings of Exchange', 'it-l10n-exchange-addon-abandoned-carts' ); ?><br /><br /></p><hr />
		<p><pre>[exchange-abandoned-carts display="cart_products"]</pre><?php _e( 'Replaced with a list of the products in the cart along with their prices.', 'it-l10n-exchange-addon-abandoned-carts' ); ?><br /><br /></p><hr />
		<p><pre>[exchange-abandoned-carts display="cart_link_href"]</pre><?php _e( 'Replaced with a unique URL for each customer\'s cart. Use inside the href tag of a link.', 'it-l10n-exchange-addon-abandoned-carts' ); ?><br /><br /></p>
		<?php
	}
}
