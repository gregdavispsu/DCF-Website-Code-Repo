...
function it_exchange_abandoned_carts_send_email_for_cart( $abandoned_cart, $email_id ) {
	...
	$GLOBALS['it_exchange']['abandoned_carts']['shortcode_data'] = array(
		'customer_name'       => empty( $user->data->display_name ) ?  __( 'Customer', 'it-l10n-exchange-addon-abandoned-carts' ) : $user->data->display_name,
		'customer_first_name' => empty( $first_name ) ?  __( 'Customer', 'it-l10n-exchange-addon-abandoned-carts' ) : $first_name,
		'customer_last_name'  => empty( $last_name ) ?  __( '', 'it-l10n-exchange-addon-abandoned-carts' ) : $last_name,
...
function it_exchange_get_abanonded_cart_status_label( $abandoned_cart ) {
	...
	if ( empty( $abandoned_cart->ID ) )
		$abandoned_cart = it_exchange_get_abandoned_cart( $abandoned_cart );
	if ( ! is_object( $abandoned_cart ) || 'IT_Exchange_Abandoned_Cart' != get_class( $abandoned_cart ) )
		return __( 'Unknown', 'it-l10n-exchange-addon-abandoned-carts' );

	switch( $abandoned_cart->cart_status ) {
		case 'abandoned' :
			$label = __( 'Abandoned', 'it-l10n-exchange-addon-abandoned-carts' );
			break;
		case 'reengaged' :
			$label = __( 'Reengaged', 'it-l10n-exchange-addon-abandoned-carts' );
			break;
		case 'recovered' :
			$label = __( 'Recovered', 'it-l10n-exchange-addon-abandoned-carts' );
			break;
		case 'expired' :
			$label = __( 'Expired', 'it-l10n-exchange-addon-abandoned-carts' );
			break;
		case 'lost' :
			$label = __( 'Lost', 'it-l10n-exchange-addon-abandoned-carts' );
			break;
		default:
			$label = __( 'Unknown', 'it-l10n-exchange-addon-abandoned-carts' );
	}
...
function it_exchange_get_abandoned_cart_email_human_readable_schedule( $email_id ) {
	...
	if ( ! empty( $scheduling['int'] ) && ! empty( $scheduling['unit'] ) )
		$value = sprintf( _n( '1 ' . rtrim( $scheduling['unit'], 's' ), "%d " . $scheduling['unit'], $scheduling['int'], 'it-l10n-exchange-addon-abandoned-carts' ), $scheduling['int'] );
	else
		$value = __( 'Unknown', 'it-l10n-exchange-addon-abandoned-carts' );
